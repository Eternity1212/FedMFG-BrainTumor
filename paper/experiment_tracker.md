...
### 多 seed 全套件 seed42 阶段性结果（2026-06-24，full_suite_v2，CPU）

seed42 已完成 fedmfg/fedamm/fedmm；fedtgp 训练中（下表为其 round 8 临时值）；fedproto 历史文件为空（忽略）。

| 算法 | 样本加权 Acc | 客户端宏平均 Acc | **客户端宏平均 Macro-F1** | BraTS(3D全模态) | Shanghai(3D缺模态) | Figshare(2D) | Brisc(2D) |
|---|---|---|---|---|---|---|---|
| fedmm | 78.12 | 71.93 | 69.49 | 50.0 | 77.5 | 79.6 | 80.6 |
| fedamm | 77.81 | 72.29 | 69.87 | 60.0 | 70.0 | 79.2 | 80.0 |
...
**分析**：
1. **两种口径结论相反**：样本加权下 2D 客户端占 ~87% 权重，fedtgp/fedmm/fedamm（77~78）> fedmfg（74）；但在公平的**客户端宏平均 Macro-F1** 下 **FedMFG 最优（73.76）**，比 fedtgp 高 ~1 分、比 fedmm/fedamm 高 ~4 分。
2. **最难客户端 FedMFG 碾压**：BraTS（3D 四模态）上 fedmfg=72.5，而 fedmm=50.0（随机水平）、fedamm/fedtgp=60.0；FedMFG 是唯一把最难 3D 客户端救到 72.5 的方法（领先 12.5~22.5 分），直接支撑"模态感知融合 + 原型指导头聚合"的核心论点。
3. **代价在 2D 大客户端**：Figshare/Brisc 上 fedmfg 略低（70.8/76.2 vs 79~86），即牺牲简单大样本 2D 来救回又难又少样本的 3D——正是本文针对的异构/模态缺失场景，体现为客户端宏平均口径下整体最优。
4. **尚不能定论**：仅 1 seed（±0 非真实方差）、fedtgp 未跑完、CPU 低分辨率缩水数据 ~10 轮；可信论文数字需多 seed 跑完 + GPU 全分辨率复现。

报告脚本：`python3 paper_tools/report_final.py --history_dir paper_outputs/public_4client/histories`（双口径 + 自动判定客户端宏平均 Macro-F1 最优方法）。

### 低正则化调参扫描结果（2026-06-25，mfg_lowreg/mfg_alpha，CPU seed42）

动机：原版 FedMFG 在客户端宏平均口径已最优（73.76），但样本加权口径落后基线；怀疑原型/头/教师三项正则过强，压制了 2D 大客户端。遂做正则强度与个性化头扫描。**所有 13 个 seed42 结果统一对比（report_final.py）：**

| 算法 | 样本加权 Acc | 客户端宏 Acc | **客户端宏 Macro-F1** | BraTS | Shanghai | Figshare | Brisc |
|---|---|---|---|---|---|---|---|
| **fedmfg-lowregmid** | **82.03** | **79.53** | **79.30** | **80.0** | 72.5 | 82.5 | 83.1 |
| fedproto（最强基线） | 81.25 | 78.02 | 77.95 | 70.0 | 80.0 | 73.3 | 88.8 |
| fedmfg（原版） | 74.06 | 74.27 | 73.76 | 72.5 | 77.5 | 70.8 | 76.2 |
| fedtgp | 78.44 | 74.24 | 72.71 | 60.0 | 80.0 | 70.4 | 86.6 |
| local | 76.09 | 72.81 | 71.66 | 60.0 | 77.5 | 75.0 | 78.8 |
| fedamm | 77.81 | 72.29 | 69.87 | 60.0 | 70.0 | 79.2 | 80.0 |
| fedmm | 78.12 | 71.93 | 69.49 | 50.0 | 77.5 | 79.6 | 80.6 |
| fedmfg-lowregstrong | 79.06 | 71.54 | 67.33 | 50.0 | 75.0 | 74.6 | 86.6 |
| fedmfg-a07 | 77.19 | 69.53 | 66.02 | 57.5 | 60.0 | 83.8 | 76.9 |
| fedmfg-a07lowreg | 73.28 | 69.27 | 65.00 | 55.0 | 72.5 | 75.8 | 73.8 |
| fedmfg-a03 | 69.53 | 64.17 | 62.63 | 52.5 | 62.5 | 67.9 | 73.8 |
| fedmfg-noteacher | 进行中（仅 3 轮） | - | - | - | - | - | - |
| fedmfg-lowregmid seed43 | 进行中（仅 1 轮） | - | - | - | - | - | - |

**lowregmid 配置**：proto_lambda=0.05 / head_lambda=0.05 / teacher_lambda=0.3（原版均为 0.1/0.1/1.0），其余不变。

**关键结论（这是能成稿的信号）**：
1. **FedMFG 调参后从"仅宏平均最优"变成"两种口径全面第一"**：lowregmid 在样本加权(82.03)、客户端宏 Acc(79.53)、客户端宏 F1(79.30) 三个口径同时超过最强基线 FedProto(81.25/78.02/77.95)，彻底消除了原版"加权口径落后"的短板。
2. **同时救活最难的 3D 客户端**：BraTS 上 lowregmid=80.0，远高于 FedProto/fedmm/fedamm/lowregstrong（50~70），保留了 FedMFG 的核心优势——既能照顾少样本难 3D，又不再牺牲 2D 大客户端。
3. **正则强度存在甜点**：mid(0.05/0.05/0.3) 最佳；strong（更小）反而崩到 67.33，说明不是"越弱越好"，而是适度放松——这正好可写成方法分析/敏感性实验。
4. **个性化头(alpha)方向被证伪**：a03/a07/a07lowreg 全部下滑（62~66），共享全局原型指导头优于混入本地头，简化了方法。

**可靠性边界**：仍是单 seed、CPU 低分辨率缩水数据 ~10 轮，±0 不是真实方差。**结论方向可信，论文数字必须等 GPU 全分辨率 + 多 seed 复现**（已交付 `experiments/gpu_launch.sh` 一键跑）。

### ⚠️ 多 seed 复核：lowregmid 的反超不稳定（2026-06-26，重要修正）

lowregmid 三个 seed（42/43/44，各 10 轮，CPU 低分辨率）全部跑完后复核：

| 方法 | seeds | SampleW Acc | Client-Macro Acc | **Client-Macro F1** | BraTS 方差 |
|---|---|---|---|---|---|
| fedmfg-lowregmid | **3** | 78.18±2.80 | 73.93±4.37 | **70.92±6.26** | ±14.14 |
| fedproto（仅 seed42） | 1 | 81.25 | 78.02 | 77.95 | — |
| fedmfg-lowregmid（仅 seed42） | 1 | 82.03 | 79.53 | 79.30 | — |

**结论修正（必须诚实）**：
1. **seed42 的 79.30 有运气成分**。seed43/44 明显更低，三 seed 平均掉到 **70.92，标准差 ±6.26**（BraTS 单客户端 ±14）。之前"FedMFG 反超所有基线"的结论**仅在 seed42 成立，不是稳定优势**。
2. **根因是 CPU 低分辨率把测试集缩得太小**（BraTS 测试仅 ~10 例，错 1 例=10% 波动），导致方差极大、单 seed 不可信。这正好坐实"CPU 低分辨率结果不能作为论文数字"。
3. **尚不能定论 FedMFG 输给 FedProto**：基线目前也只有 seed42；FedProto 在 seed43/44 上可能同样下滑。已启动 `run_finish_all_cpu.sh` 补齐基线三 seed，做 apples-to-apples 对比。

### 下一步迭代方向（基于多 seed 现实重新规划）

1. **停止在 CPU 上调超参**——方差太大，等于在拟合噪声，调出来的"甜点"换个 seed 就没了。CPU 只用来验证代码链路与方向，不产出论文数字。
2. **GPU 全分辨率是唯一可靠路径**：测试集大 → 方差小 → 结论可信。已交付 `experiments/gpu_launch.sh`（lr=3e-4 + lowregmid + seed42/43/44 全分辨率）。
3. **论文真正的差异化卖点是"缺失模态鲁棒性"，不是干净数据上的绝对精度**。FedMFG 的核心主张是处理模态缺失：即使在干净数据上与 FedProto 打平，只要**随模态缺失比例上升、FedMFG 退化得更慢**，论文就立得住。缺失模态开关已实现（`--missing_modality_rate_*`），GPU 脚本 STAGE 3 会自动跑 0/25/50/75% 曲线——这应作为主图。
4. **消融已有正向信号**：去教师原型后测试 Acc 0.82→0.70 下降，支持组件有效性；个性化头方向已证伪（用 alpha=0）。
